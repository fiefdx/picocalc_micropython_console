...
class TarFile:
    def __init__(self, name=None, mode="r", fileobj=None):
        ...
        if fileobj:
            ...
            f = open(name, modes[mode])
        # for gzip support
        if mode == "r" and name and name.endswith(".gz"):
            # AUTO = detect gzip/zlib/raw, 15 = max window
            f = deflate.DeflateIO(f, deflate.AUTO, 15)
        self.f = f
        self.subf = None
        self.mode = mode
        self.offset = 0
...
#         d.size = int(bytes(h.size), 8)
        d.size = int(bytes(h.size).rstrip(b"\0 ").decode(), 8)
