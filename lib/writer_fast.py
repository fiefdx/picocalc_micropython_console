...
#         del fbc
    ...
    def clear_line(self, length, invert=False):
        s = self._getstate()
        self.device.rect(s.text_col, s.text_row, self.char_width * length, self.char_height, self.fgcolor if invert else self.bgcolor, True)
    ...
    def _printchars(self, char, invert=False, recurse=True):
        ...
        if char not in Writer.chars:
            buf = bytearray(self.glyph)
            Writer.chars[char] = buf
#             Writer.chars[char] = framebuf.FrameBuffer(buf, self.clip_width, self.char_height, self.map)
        else:
            buf = Writer.chars[char]
#         buf = Writer.chars[char]
        if invert:
        ...
        #tttt = ticks_ms()
        for p in self.char_map[char]:
            self.device.blit(fbc, p * self.char_width + s.text_col, s.text_row, -1, palette)
            #s.text_col += self.char_width
        #self.cpos += 1
        #ttttt = ticks_ms()
        #print("_printchar: ", ttttt - t, ttttt - tttt, tttt - ttt, ttt - tt, tt - t)
